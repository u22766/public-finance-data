...
│
└── tests/
    ├── validate.py                              ← Core validation (1,052 checks)
...
All data files are validated on every push and pull request via GitHub Actions. The CI pipeline runs fifteen test suites totaling **5,880 checks**:

| Suite | File | Checks | Coverage |
|-------|------|--------|----------|
| Core | `validate.py` | 1,052 | Manifest integrity, all federal/state/reference files, pension systems, state benefits audit guard rails, legislation watch, partial exemption audit |
